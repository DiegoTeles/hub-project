...
export default class request {
  ...
  request(options: object = {}): Promise<any> {
    let promise = new Promise((resolve, reject) => {
      try {
        ...
        axios(basePrefix(this.settings.url, request)).then(
          (response) => {
   ...
   * @param {object} header  - Header to be posted.
   * @param {object} [query] - Optional request parameters.
   *
   * @returns {Promise}
   */
  post(path: string, data: object, header?: any, query?: object): Promise<any> {
    let request = {
      method: 'post',
      header,
      url: path,
